  ...
  def verifyJQToken(jwt: String) = {
    val jwtConsumer = new JwtConsumerBuilder()
      ...
      .setJwsAlgorithmConstraints( // only allow the expected signature algorithm(s) in the given context
        new AlgorithmConstraints(
          ConstraintType.WHITELIST, // which is only RS256 here
    ...
    Try(jwtConsumer.processToClaims(jwt)) match {
      ...
      case Failure(ex: InvalidJwtException) =>
        ...
        if (ex.hasExpired)
          println(
            "JWT expired at " + ex.getJwtContext.getJwtClaims.getExpirationTime
          )
        if (ex.hasErrorCode(ErrorCodes.AUDIENCE_INVALID))
          println(
            "JWT had wrong audience: " + ex.getJwtContext.getJwtClaims.getAudience
          )
